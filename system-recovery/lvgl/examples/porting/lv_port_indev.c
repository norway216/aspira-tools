...
static lv_indev_t *mouse_indev;  // LVGL输入设备
...
void mouse_init(void)
{
    /*Your code comes here*/
    // 尝试打开鼠标设备
    const char *mouse_device = "/dev/input/event2";  // 可能需要修改设备节点
    ...
    // 初始化鼠标位置
    mouse_x = LV_HOR_RES_MAX / 2;
    mouse_y = LV_VER_RES_MAX / 2;
...
void mouse_read(lv_indev_drv_t *drv, lv_indev_data_t *data) {
    ...
    while (read(mouse_fd, &ev, sizeof(struct input_event)) > 0) {
        switch (ev.type) {
            case EV_REL:  // 相对移动事件
                if (ev.code == REL_X) {
                    mouse_x += ev.value;
                    ...
                    if (mouse_x >= LV_HOR_RES_MAX) mouse_x = LV_HOR_RES_MAX - 1;
                }
                else if (ev.code == REL_Y) {
                    mouse_y += ev.value;
                    // 限制鼠标范围
                    if (mouse_y < 0) mouse_y = 0;
                    if (mouse_y >= LV_VER_RES_MAX) mouse_y = LV_VER_RES_MAX - 1;
                }
                break;

            case EV_KEY:  // 按键事件
                if (ev.code == BTN_LEFT) {
                    mouse_left_pressed = ev.value;
                }
                break;
        }
    }
